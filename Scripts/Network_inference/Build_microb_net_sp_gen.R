...
constructAndSaveNetwork <- function(networkType, data, thresh, taxRank, seed=123, 
    ...
    net <- do.call(netConstruct, c(list(data), network_params))
...
option_list <- list(
  ...
  make_option(c("-b", "--biome"), type="character", default="PGE",
...
valid_biomes <- c("PGE_sp", "NOPGE_sp", "PGE_gen", "NOPGE_gen")
...
if (opt$biome %in% c("PGE_sp", "NOPGE_sp")) {
  current_tax_rank <- "Species_Name"
} else if (opt$biome %in% c("NOPGE_gen", "PGE_gen")) {
...
file_input_path <- paste0("Data/", opt$biome, "_phyloseqfile.rds")

# Load data
phyloseq_species <- readRDS(file_input_path)
