...
def calculate_network_metrics(
    ...
    avg_clustering_coefficient = nx.average_clustering(G, weight=weight_column)
...
def process_all_biomes(base_dir: str, network_type: str, output_dir: str, weight_column: str) -> Tuple[pd.DataFrame, Dict[str, Tuple[Union[nx.Graph, None>, Union[str, None]]]]:
    """
    Processes each biome's networks within the base directory, consolidates their metrics,
    and identifies the best network for each biome. This unified DataFrame is then saved as a CSV file.

    Parameters
    ----------
    base_dir : str
        The base directory containing subdirectories for each biome.
    network_type : str
        The type of network (e.g., 'cclasso', 'spring') to process.
    output_dir : str
        The directory where the unified metrics CSV and best network clustering information will be saved.
    weight_column : str
        The column to use as edge weight for network metrics ('adja', 'asso', or 'diss').

    Returns
    -------
    Tuple[pd.DataFrame, Dict[str, Tuple[Union[nx.Graph, None], Union[str, None]]]]
        A tuple containing:
        - pd.DataFrame: A unified DataFrame containing metrics for all biomes.
        - Dict[str, Tuple[Union[nx.Graph, None], Union[str, None]]]: A dictionary mapping biome names
          to a tuple of (best_network_graph, best_threshold) for that biome.
    """
    
    all_biomes_metrics: List[pd.DataFrame] = []
    biome_best_networks: Dict[str, Tuple[Union[nx.Graph, None], Union[str, None]]] = {}
    

    for biome in os.listdir(base_dir):
        
        biome_path: str = os.path.join(base_dir, biome)
        
        if os.path.isdir(biome_path):
            print(f"\nProcessing biome: {biome}")
            
            df_biome, best_network_graph, best_threshold = process_biome_networks(
                biome, biome_path, network_type, weight_column
            )
            
            if not df_biome.empty:
                all_biomes_metrics.append(df_biome)
                biome_best_networks[biome] = (best_network_graph, best_threshold)
                print(f"Finished calculating metrics for biome: {biome}")

    # Combine all biome DataFrames into one unified DataFrame
    combined_df: pd.DataFrame = pd.concat(all_biomes_metrics, ignore_index=True)
    
    # Ensure output directory for unified metrics exists
    os.makedirs(output_dir, exist_ok=True)

    # Save the unified DataFrame as a CSV file
    output_csv_path: str = os.path.join(
        output_dir, f"{network_type}_{weight_column}_metrics_all_biomes.csv" # Adjusted filename
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform topological analysis on microbial association networks generated with "
                    "the NetComi package, looping over various biomes and thresholds."
    )
    parser.add_argument(
        "--base_dir",
        type=str,
        required=True, # Made required
        help="Base directory containing subdirectories for each biome.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        required=True, # Made required
        help="Directory to save the analysis results (unified metrics CSV and best network clustering).",
    )
    parser.add_argument(
        "--network_type",
        type=str,
    ...
    os.makedirs(os.path.join(args.output_dir, "Best_nets/Clustering"), exist_ok=True)


    # Process all biomes, get combined metrics and best networks per biome
    clust_metrics_combined, biome_best_networks = process_all_biomes(
    ...
    for biome, (best_network_graph, best_threshold) in biome_best_networks.items():
        if best_network_graph:
            print(
                f"\nExporting clustering for best network of biome: {biome} at threshold: {best_threshold}"
            )

            # Ensure weights are absolute for best network clustering, as some metrics do not support signed values.
            # This transformation applies if the original weight column has signed values (like 'asso').
            ...
            clustering_output_path: str = os.path.join(
                args.output_dir,
                "Best_nets/Clustering",
                f"{biome}_best_net_{best_threshold}_node_clustering_{args.weight_column}.csv", # Adjusted filename
            )
            
            df_node_community.to_csv(clustering_output_path, index=False)
            print(
                f"Node clustering information for {biome}'s best network saved to {clustering_output_path}"
            )
        else:
            print(f"\nNo single best network could be determined for biome: {biome} based on the criteria.")